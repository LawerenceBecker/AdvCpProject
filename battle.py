...
class Battle:
  def __init__(self, player, pokemon2, trainerInfo=None):
    ...
    self.playerSTicks = pygame.time.get_ticks()
    self.cpuSTicks = pygame.time.get_ticks()
    ...
    while battle:
        ...
        for event in event_list:
            ...
            # should open menu here
              ...
              if pygame.time.get_ticks() - self.playerSTicks >= poke1special.cooldown and self.playerChargeMeter >= poke1special.meterSize:
        ...
        if pygame.time.get_ticks() - self.cpuTicks >= poke2quick.cooldown:
          ...
          if pygame.time.get_ticks() - self.cpuSTicks >= poke2special.cooldown and self.cpuChargeMeter >= poke2special.meterSize:
            damage = self.damage(poke2special,self.pokemon2,self.pokemon1)
            self.cpuChargeMeter -= (poke2special.meterSize + poke2quick.fillMeter)
